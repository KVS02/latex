...
\begin{lemma}
  The map $\bar K^\times/\bar O_K^\times \to \oplus_v \bar K_v^\times/\bar O_{K_v}^\times$ induces an isomorphism \[(\bar K^\times/\bar O_K^\times)^{G_K} \to \oplus_v (\bar K_v^\times/\bar O_{K_v}^\times)^{G_{K_v}}.\]
\end{lemma}
\begin{proof}
  The first map is clearly injective. The RHS is just $\oplus_v \Q$ under the valuation maps $K_v \to \Z$, and this product is $G_{K_v}$-invariant already because Galois elements preserve valuation. So we just need surjectivity of the original map. Take $(1/m_v)_v \in \osum_v \Q$. Then take the ideal $\prod_v \p_v^{m_v}$ in $\bar O_K$, and choose a Galois extension $L/K$ in which this becomes principal, say $(x_m) = \prod p_v^{m_v}$. Then the class of $x_m \in \bar K_v^\times$ in $\osum_v \bar K_v^\times/ \bar O_{K_v}^\times$ is precisely $(1/m_v)_v$, and $x_m$ is Galois invariant modulo units.  
\end{proof}
...
\[\begin{tikzcd}
	...
	0 & {\oplus_v K_v^\times/O_{K_v}^\times} & { \oplus_v (\bar K_v^\times/\bar O_{K_v}^\times)^{G_{K_v}}} & {\oplus_v H^1(K_v, \bar O_{K_v}^\times)} & 0 \\
	& {\Cl(K)} & 0
