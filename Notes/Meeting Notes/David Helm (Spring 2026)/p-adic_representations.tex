...
Converse of Schur's lemma does not hold. The space $\Hom_G(\Ind \mathbf 1, \Ind \delta_B^{-1})$ is one dimensional but the representations are not isomorphic (or irreducible). The non-zero functions in this space has kernel the trivial representation and image the dual of Steinberg representation, inducing an isomorphism $\mathrm{St} \simeq \mathrm{St}^\vee$.\\

One can see that the sequence \[0 \to \chi^w \delta_B^{-1} \to (\Ind \mathbf 1 )_N \to \chi \to 0\] splits for $\chi$ not self-dual explicitly by $\chi \hookrightarrow \Ind \mathbf 1$ as $B$-representations. Alternatively the extension group $\Ext^1(\chi_1, \chi_2)$ is trivial for distinct characters.\\

Let $f$ be a Hecke eigenform of level $N$ and let $p$ be a prime not dividing $N$. Consider the span of the forms $f(p^iz), i \geq 0$ in the space of modular forms. This subspace has an action of $\GL_2(\Q_p)$, and the representation is exactly a principal series representation $\Ind_B^G \chi$, where $\chi$ is a character related to the $T_p$-eigenvalue of $f$. (Fill in more details).\\

The plan is to now move to functional equation for $\GL_2$ and see Langland's correspondence.

\section{Meeting 8}
More details on the action of $\GL_2(\Q_p)$. Representation is a principal series because it contains a $N$-fixed vector.\\

Local global compatibility. Restrict global representation to decomposition group of a prime to get the local group.

Hilbert modular varieties. Modular curve is the moduli space of elliptic curves, level structure. For $F$ a totally real field, the automorphic forms are functions on lattices $\Lambda$ in $\C^d$, and the quotient $A = C^d/\Lambda$ makes it an abelian variety. (There is a Hermitian form on $\C^d$ which gives a dual lattice and polarization of $A$.) Hilbert modular varieties are moduli space of abelian varieties of dimension $d$ with a polarization and $O_F$-action. Elliptic curves have a canonical polarization and a $\Z$-action. Hilbert modular forms are sections of bundles on the modular variety. (Example of Shimura variety for $\GL_2/F$.)\\

\section{Meeting 9-12}
Local L-factor of an irreducible representation of Weil group $W_F$. For one dimensional representations, it is defined by Tate's thesis, and for higher dimensions, it is $1$. There is a definition for any smooth representation by the inverse of the determinant of $1 - q^{-s}\Phi_F$ acting on the inertia fixed points of $V$. For higher dimension representations, $\Phi$ acts on $V^{I_F}$ since $\Phi$ acts on $I_F$. So by irreducibility, this representation is either unramified or $V^{I_F}$ trivial. In the unramified case, the Frobenius acts semisimply so irreducibility implies that the representation is one dimensional. For $\dim V \geq 2$ irreducible, we thus have $V^{I_F} = 0$ so the L-factor is $1$.\\

[Tate's thesis and Zeta functions on curves] Is there a relation between zeta function on curves and the zeta integrals from Tate's thesis?\\ For higher dimensional varieties over finite fields, one needs etale cohomology to prove functional equation. $\ell$-adic cohomology cup product pairing for curves is the Weil pairing for their Jacobians: $H^1_{\text{\'et}}(X, \Q_\ell) = T_\ell(\Jac(X))$.

Choices of additive character in Laurent series local fields $\F_q((T))$: take $T^{-1}$ to a fixed primitive $q$-th root of unity $\zeta$, and trivial on all other powers of $T$. The global adelic additive character coming from these local characters is trivial on the function field by residue theorem. Something about Artin-Schreier extensions relating extensions of the field to finite covers of the curve.\\

The Galois action on the Langlands $L$-group: The dual group $G^\vee$ comes with a canonical pinning. We have an exact sequence \[1\to \Inn(G^\vee) \to \Aut (G^\vee) \to \Aut (BRD(G^\vee)) \to 1.\] The pinning gives a splitting of this sequence. There is a Galois action on based root data, which via the pinning induces a Galois action on $G^\vee$. \\
Let $G^\vee/F$ be quasi split. Choose Borel $B/F$ and a maximal torus $T'/F$. Then there is a Galois action on $ T = T'_{\overline F}$, which induces a Galois action on based root datum (one can check the Galois action preserves root systems: the weights of adjoint action of $T$ on $\g$ are permuted by the Galois action - fixed field related to the field over which $T$ splits(?).) \\
This Galois action differentiates $F$-forms of $G(^\vee)$ with the same Langlands dual group, and the identification is upto inner forms (inner forms have similar representation theory).\\ The choice of torus is unique upto conjugation by $G/Z(F)$, which is not the same as $G(F)/Z(F)$. This is true when $F$ is algebraically closed. In general taking invariants is only left exact so $G(F)/Z(F) \subset G/Z(F)$. Sheafification of the presheaf $G(R)/Z(R)$ in fppf (maybe etale in weaker cases) gives a quasi projective variety $G/Z$. Example, $\SL_2(\R) \to \P\GL_2(\R)$ is not surjective, one must go to an  etale cover $\C/\R$ to make this map surjective.
