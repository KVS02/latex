...
\title{Local Langlands Correspondence for $\GL_n/\R$}
\author{Sudharshan K V}
\date{\today}
\begin{document}
\maketitle
\begin{abstract}
  These are the notes for my short talk for the finals of Geometric Representation Theory (M392C) taught by Lucas Mason Brown.
\end{abstract}
\section{Introduction}
Let $F$ be a local field. Roughly, the Local Langlands Correspondence (LLC) for $\GL_n/F$ is a bijection between the $n$-dimensional (+ adjectives) representations of the Weil group $W_F$ of $F$ [Galois side] and the irreducible (+ adjectives) representations of $\GL_n(F)$ [Automorphic side]. The Weil group of a local field is a modification of its absolute Galois group. In this note, we will describe the LLC for the local field $\R$. In fact, the archimedean case motivated Langlands to consider such potential relations over non-archimedean local fields, leading to the formulation of the LLCs!\\

The representation theory of $W_\R$ is quite simple. On the other hand, the classification of irreducible admissible representations of a real reductive group upto infinitesimal equivalence was achieved by Langlands (\cite{langlands}), and is called the Langlands classification. The LLC for $\GL_n/\R$ is evident from the Langlands classification for $\GL_n$, once the representations of $W_\R$ are identified. 

\section{The Weil group of $\mathbb R$}
For any local field $F$, one can define the Weil group $W_F$ axiomatically (see \cite{tate}). The Weil group for $\R$ is a non-split extension of $\Gal(\C/\R)$ by $\C^\times$. Concretely, we have $$W_\R = \C^\times \cup j\C^\times,$$ where $j^2 = -1, jzj^{-1} = \bar z$ for $z \in \C^\times$. These are the units of the quarternion algebra: the non-split extension is given by the non-trivial element of the Brauer group of $\R$ (which is $\Z/2\Z$), and this element corresponds to a non-split central simple algebra over $\R$ - the quarternions. We will however, work with the concrete description.
...
\[jzj^{-1}z^{-1} = \bar z z^{-1},\] so the commutator subgroup is the unit circle $S^1$ in $\C^\times$. $\C^\times/S^1$ is the positive real numbers, and we can identify $jC^\times/S^1$ with the negative reals. Therefore, $W_\R^{\text{ab}} = \R^\times$. This is one of the axioms in \cite{tate}.\\
...
\begin{theorem}\label{thm:llcgl1}[LLC for $\GL_1/\R$] There is a bijection between irreducible (admissible) representations of $\GL_1(\R)$ and the one dimensional (semisimple) representations of the Weil group $W_\R$.
\end{theorem}
...
Next, consider the \emph{relative discrete series} representations $\sigma_{l,t}$ of $\GL_2(\R)$, given by \[\sigma_{l,t} = D_l \otimes |\det|^t,\] where $l \geq 1$ and $t \in \C$. These representations will be the building blocks (in rank 2) for representations of $\GL_n(\R)$. (Relate this to the representation

\subsection{Langlands Classification for $\GL_n$}
We state Langlands' classification of irreducible admissible representations of $\GL_n(\R)$ upto infinitesimal equivalence. There is a more general version for an arbitrary reductive group in place of $\GL_n$, but we will not state that here (see \cite{getzhahn}, Chapter 4).

For any partition of $n = \sum_{i=1}^r n_i$ where $n_i = 1$ or $2$, and representations $\sigma_i$ of $\GL_{n_i}$ of the form mentioned above (i.e. characters of $\R^\times$ or the relative discrete series $\sigma_{l,t}$), let $M$ be the Levi subgroup $\prod_i \GL_{n_i}(\R)$ of $\GL_n(\R)$ and let $P$ be the standard parabolic of $\GL_n(\R)$ containing $M$. Inflate the representation $\sigma = (\sigma_1, \dots, \sigma_r)$ of $M$ to $P$, and consider the normalized induction \[I(\sigma_1,\dots,\sigma_r) = \Ind_P^{\GL_n(\R)} \left( (\sigma_1, \dots, \sigma_r) \otimes \rho_P \right ),\] where $\rho_P$ denotes the half-sum of positive roots associated to the parabolic $P$. 

\begin{theorem}[Langlands]
\begin{enumerate}
\item Suppose $t_1/n_1 \geq t_2/n_2 \geq \dots \geq t_r/n_r$, then $I(\sigma_1,\dots, \sigma_r)$ has a unique irreducible quotient denoted $J(\sigma_1,\dots,\sigma_r)$.
