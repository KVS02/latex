...
In this section, we state Langlands classification for irreducible admissible representations of $\GL_n(\R)$ upto infinitesimal equivalence. As mentioned earlier, the representations of $\GL_1(\R)$ are parameterized by a complex variable $s$ and a character $\epsilon$ of $\{\pm 1\}$. We will first discuss the classification for $\GL_2$ and then about how one builds representations of $\GL_n$ from the representations in rank $1$ and $2$.

\subsection{$n=2$}
We build the representations of $GL_2(\R)$ from the group $\SL_2^\pm(\R)$ of $2\times 2$ matrices of determinant $\pm 1$. Recall that $\SL_2(\R)$ has discrete series representations $D_l^+$ for $l$ a non-zero integer, $|l| \geq 2$. There are also the two limits of discrete series representations $D_{\pm 1}^+$.

We consider the discrete series (and limit of discrete series) representations of $\SL_2^\pm (\R)$ given by \[D_l = \Ind_{\SL_2(\R)}^{\SL_2^\pm(\R)}(D_l^+), l \geq 1.\]
